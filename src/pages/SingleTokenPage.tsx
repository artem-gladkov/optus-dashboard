...
const SingleTokenPageComponent = ({ typePage }: Props) => {
      ...
      if (typeButtonCharts === "Liquidity") {
        return (
          <ChartsOverview
            ...
            titleMarker={false}
            type={"Liquidity"}
            data={toJS(getSingleToken.liquidity_graph)}
          />
        );
      }
      if (typeButtonCharts === "Volume") {
        return (
          <ChartsOverview
            ...
            titleMarker={false}
            type={"Volume (24hrs)"}
            data={toJS(getSingleToken.volume_graph)}
          />
        );
      }
      ...
      {getSingleToken.address ? (
        getErrorSingleToken ? (
          ...
          <div className="h-full py-14 bg-bg flex flex-col justify-center relative">
            <div className="h-full flex flex-col justify-center relative mt-20">
              <Bg />
            </div>
            <div className="flex w-full h-full flex-col text-text lg:px-28 px-4 z-50 -mt-20">
              <div className="flex justify-between w-full  flex-wrap  items-center">
                <div className="flex">
                  ...
                  <span className="ml-6">
                    {getSingleToken.symbol}
                    <span className="ml-2">&#8250;</span>
                  </span>
                  <Link
                    to={`https://tonapi.io/account/${getSingleToken.address}`}
                    className="ml-1 w-28"
                    target="_blank"
                  >
                    ({getSingleToken.address.slice(0, 6)}...
                    {getSingleToken.address.slice(-6)})
                  </Link>
                </div>
              ...
              <div className="flex justify-between items-end mt-10">
                <div className="flex items-end ">
                  ...
                  <div className="lg:text-3xl text-xl ml-2 hidden md:block">
                    {getSingleToken.name}
                  </div>
                  <div className="lg:text-3xl text-xl ml-2">
                    ({getSingleToken.symbol})
                  </div>
                  <div className="lg:text-xl text-base ml-2">
                    {getSingleToken.current_usd_price.value}$
                  </div>
                  <div
                    className={`${numberColor(
                      getSingleToken.current_usd_price.change
                    )} ml-2 font-bold`}
                  >
                    {getSingleToken.current_usd_price.change} %
                  </div>
              ...
              <div className="flex flex-col mt-10">
                ...
                <div className="md:flex md:justify-between w-full mt-3 ">
                  ...
                  <div className="md:flex flex-col w-4/12 mr-2  rounded-3xl justify-between hidden">
                    <TokenForm
                      title={"Liquidity"}
                      value={getSingleToken.liquidity.value}
                      change={getSingleToken.liquidity.change}
                    />
                    <TokenForm
                      title={"Volume (24hrs)"}
                      value={getSingleToken.volume_24h.value}
                      change={getSingleToken.volume_24h.change}
                    />
                    <TokenForm
                      title={"Transactions (24hrs)"}
                      value={getSingleToken.transactions_24h.value}
                      change={getSingleToken.transactions_24h.change}
                      transaction={true}
                    />
                  </div>
              ...
              <div className="flex flex-col mt-5">
                ...
                <TokenInformation
                  symbol={getSingleToken.symbol}
                  name={getSingleToken.name}
                  address={getSingleToken.address}
                />
              </div>
            </div>
          </div>
