...
const TokensComponent = ({ typePage }: Props) => {
      ...
      return (
        ...
        >
          ...
          <div className="flex lg:w-2/3 w-full text-xs sm:text-base font-medium">
            ...
            <div className="flex md:w-1/5 w-1/3  justify-center md:justify-start">
              <span>{token.native_liquidity.value} </span>
            </div>
            <div className="flex md:w-1/5 w-1/3 px-2 justify-center  md:justify-start">
              <span>{token.volume_24h.value} $</span>
            </div>
            <div className="md:flex md:w-1/5 hidden  md:flex-row justify-center  md:justify-start">
              <span className="flex">{token.current_usd_price.value} $</span>
            </div>
            <div
              className={`md:flex md:w-1/5 hidden  md:flex-row justify-end  md:justify-start ${numberColor(
                token.current_usd_price.change
              )}`}
            >
              <span className="flex">{token.current_usd_price.change} %</span>
            </div>
          </div>
        </div>
      );
    ...
    <div className="w-full border rounded-2xl mt-4 bg-form border-inActive text-xs sm:text-base">
      ...
      <div className="flex flex-col items-center xl:flex-row w-full">
        ...
        <div className="flex  items-center w-full xl:w-1/3 justify-center">
          <Pagination
            totalItem={store.getTokens.length}
