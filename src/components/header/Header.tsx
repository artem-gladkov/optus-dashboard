            ...
            {matches ? (
                     <Navigations/>
                         ...
                         <div className="flex flex-end text-inActive">
                              ...
                              <div className={burger ? 'bg-form  mt-10 absolute right-0  z-50 text-text' : 'hidden'}>
                       {matches ? ( null): (
                            <div className="flex flex-col text-inActive rounded border">
                                 <SearchInput hiddenNav={(hidden: boolean)=>{hiddenNav(hidden)}}/>
                                 <div className={`${hidden ? 'hidden' : ''}`}>
                                    <div className="mb-2">
                                    <Link onClick={()=>{updateHandlerButtonDexBo(true);updateActivePage('')}} className=' hover:text-text transition-all duration-400 px-4 py-2 rounded-xl' to={`/${dex ? dex : over}`}>Overview</Link>
                                  </div>
                                  <div className="mb-2">
                                    <Link onClick={()=>{updateActivePage(tokens)}} className=' hover:text-text transition-all duration-400 px-4 py-2 rounded-xl' 
                                                  to={`/${dex ? dex : over}/${tokens}`}>Tokens</Link>
                                  </div>
                                  <div className="mb-2">
